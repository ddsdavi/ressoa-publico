-- Pontuação v1.3 — compra aprovada, compra repetida e reembolso.
-- (mantém o conserto da v1.2 sobre o resíduo da importação)
--
        ...
        case r.tipo
          ...
          when 'clicou_email' then
            case when exists (select 1 from public.eventos_email e
                              where e.lead_fk = l.lead_id and e.tipo = 'click'
                                and e.occurred_at > now() - (r.dias || ' days')::interval)
                 then r.pontos else 0 end
          -- só compra aprovada pontua. Reembolso e chargeback pontuando
          -- como venda colocaria quem pediu o dinheiro de volta no topo.
          when 'comprou' then
            case when exists (select 1 from public.tabela_4_alunos c
                              where c.lead_fk = l.lead_id and c.status = 'aprovada')
                 then r.pontos else 0 end
          ...
          when 'recencia' then
            case when greatest(
                 ...
                 then r.pontos else 0 end
          when 'comprou_repetido' then
            case when (select count(*) from public.tabela_4_alunos c
                       where c.lead_fk = l.lead_id and c.status = 'aprovada') > 1
                 then r.pontos else 0 end
          when 'reembolsou' then
            case when exists (select 1 from public.tabela_4_alunos c
                              where c.lead_fk = l.lead_id
...
-- regras novas, agora que a compra tem status
insert into public.regras_pontuacao (nome, tipo, pontos, dias)
select * from (values
  ('Comprou mais de uma vez', 'comprou_repetido', 35, null::int),
  ('Pediu reembolso ou chargeback', 'reembolsou', -45, null::int)
) v
where not exists (select 1 from public.regras_pontuacao where tipo = 'comprou_repetido');
